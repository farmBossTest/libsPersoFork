...
# detail:
	- makeTree : retourne un pointeur sur un arbre de profondeur targetDepth contenant coups possibles selon nextBoard (prend un etat du jeu et un entier puis renvoie un tableau alloué dynamiquement des états possible au tour suivant et met size au nombre d'états possible) depuis la position startBoard. les feuilles de l'arbres sont évaluées avec evaluate (prend un état du jeu et renvoie son score) et les autre nœuds en remontant l'arbre selon minimax. Retourne le pointeur NULL en cas d'erreur
