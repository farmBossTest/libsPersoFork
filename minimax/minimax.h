...
#define MINIMAXBOARDTYPE board_t
...
int choseBestMove(                                      // calcule le meilleur coups a joué dans la position donné par startBoard selon l'algorithme minimax
    MINIMAXBOARDTYPE startBoard,                                 // plateau a partir du quel minimax ser a appliqué
    int targetDepth,                                    // profondeur maximale de l'exploration
    MINIMAXBOARDTYPE *(*nextBoard)(MINIMAXBOARDTYPE board, int *size),    // fonction qui revoie un pointeur vers un tableau (alloué dynamiquement) des differents coups possibles 
    int(*evaluate)(MINIMAXBOARDTYPE board),                      // fonction d'evalutation d'un plateau (plus la valeur est grande meilleur est le plateau pour le joueur appelant chooseBestMove)
    MINIMAXBOARDTYPE *dest                                       // pointeur vers la variable de retour
